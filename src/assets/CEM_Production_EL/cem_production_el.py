...
class CEM_Production_EL_Asset(Stock_Asset_STEVFNs):
    """
    Time-dependent electric kiln cement produciton asset
    """
    asset_name = "CEM_Production_EL"
    source_node_type = "NULL"
    target_node_type = "CEM"                              # year-indexed cement demand node
    target_node_type_2 = "CO2_Budget"                     # period-indexed emissions node
    target_node_type_3 = "CEM_Production_EL_Capacity"     # period-indexed capacity-limit node
    el_node_type = "EL"                                   # hourly electricity node
    stock_node_type = "CEM_Production_EL_Stock"
    ...
    def get_period_emissions(self):
        """Total MtCO2e/year (process) per reinvestment period --
        feeds the standard 'emissions' metric in get_results_records."""
        return self.get_period_process_emissions()
