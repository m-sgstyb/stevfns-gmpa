...
class RE_WIND_Onshore_Lim_0_Asset(Asset_STEVFNs):
    """Class of Renewable Energy Sources """
    asset_name = "RE_WIND_Onshore_Lim_0"
    target_node_type = "EL"
    source_node_type_2 = "NULL"
    target_node_type_2 = "RE_WIND_Onshore"
    period = 1
    transport_time = 0
    ...
    def _load_RE_profile(self):
        ...
        n_sets = np.int64(np.ceil(self.number_of_edges/set_size))
        gap = np.int64(len(full_profile) / (n_sets * set_size)) * set_size
        offset = set_size * set_number
        new_profile = np.zeros(np.int64(n_sets * set_size))
        for counter1 in range(n_sets):
            old_loc_0 = np.int64(offset + gap*counter1)
            old_loc_1 = np.int64(old_loc_0 + set_size)
            new_loc_0 = np.int64(set_size * counter1)
            new_loc_1 = np.int64(new_loc_0 + set_size)
