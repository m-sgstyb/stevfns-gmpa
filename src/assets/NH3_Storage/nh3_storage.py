...
class NH3_Storage_Asset(Stock_Asset_STEVFNs):
    """Class of NH3 storage asset"""

    asset_name = "NH3_Storage"
    source_node_type = "NH3"
    target_node_type = "NH3"
    target_node_type_2 = "NH3_Storage_Capacity"
    stock_node_type = "NH3_Storage_Stock"
    ...
    def define_structure(self, asset_structure):
        ...
        self.target_node_times = np.arange(asset_structure["Start_Time"],
                                            asset_structure["End_Time"],
                                            self.period)
        # Roll target times forward by one timestep (storage carries H2
        # from hour t to hour t+1), cycles last hour back to first
    ...
    def _build_capacity_limit_edges(self):
        """Caps hourly storage throughput within each period at that
