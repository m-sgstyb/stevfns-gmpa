        ...
        for country_code, (lat, lon) in country_coords.items():
            ...
            if "profile_path" in row and pd.notna(row["profile_path"]):
                ...
                if "Demand" in asset_name:
                    ...
                    dest_dir = os.path.join(
                        asset_dir, "profiles", profile_dir_name, f"lat{r_lat}"
                    )
                    if not os.path.exists(dest_dir):
                        os.makedirs(dest_dir)
                    ...
                    print(
                        f"Warning: Unknown profile type for {asset_name}. Skipping profile copy."
                    )
            ...
            for col in params_df.columns:
                if col not in row_dict:
                    ...
                    if col == "Type":
                        if not params_df.empty and pd.api.types.is_numeric_dtype(
                            params_df["Type"]
                        ):
