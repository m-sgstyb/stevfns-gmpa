...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 19 15:18:23 2026
...
class VEH_EL_Freight_Asset(Asset_STEVFNs):
    ...
    """
    asset_name = "VEH_EL_Freight"
    source_node_type = "NULL" # Edge to freight demand node
    target_node_type = "FD_VEH" # Edge to freight demand node
    target_node_type_aux = "EV_Freight_Demand" # Edge to aux node to match units 
    ...
    def __init__(self):
        self.cost_fun_params = {"sizing_constant": cp.Parameter(nonneg=True, name=f"sizing_constant_{self.asset_name}"),
                                ...
                                "conversion_factor": cp.Parameter(nonneg=True, name=f"mtkm_GWh_conversion_factor_{self.asset_name}"),
            }
        # Energy to million tonne-km conversion
        self.conversion_fun_params = {"conversion_factor": cp.Parameter(nonneg=True, name=f"mtkm_GWh_conversion_factor_{self.asset_name}"),
            }
        return
    ...
    def define_structure(self, asset_structure):
        ...
        self.flows = cp.Variable(nonneg=True) # Size of FD provided by EV, scalar
        return
    
    def build_pd_edge(self):
        """
        Build single edge to annual freight demand node (in mtkm units)
        """
        source_node_time = 0
        target_node_time = 0
        ...
        pd_edge.conversion_fun = self.conversion_fun # Converts to million tonnes-km
        pd_edge.conversion_fun_params = self.conversion_fun_params
        return
    
    def build_ev_dem_edge(self):
        """
        Build single edge to EV annual freight demand node (in energy units)
        """
        source_node_time = 0
        target_node_time = 0
    ...
    def build_EL_edges(self, edge_number):
        """
        Build hourly edges to charge EV freight fleet based on input parameter
        """
    ...
    def _load_charging_profile(self):
        """
        Method loads user-provided hourly profile parameter for freight EV charging load shape
        Samples profile under same sampling technique for RE profiles
        """
        PROFILE_LOC = self.parameters_df["loc_name"]
        profile_filename = os.path.join(self.parameters_folder, "profiles", f"{PROFILE_LOC}_EV_Freight_Charge_GW.csv")
